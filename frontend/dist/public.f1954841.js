async function handleSearchClick() {
    ...
    try {
        ...
        if (!response.ok) throw new Error(`Fehler beim Abrufen von Podcasts: ${response.status}`);
        const data = await response.json();
        const podcasts = data.data || [];
        resultsContainer.innerHTML = '';
        if (podcasts.length === 0) resultsContainer.innerHTML = '<p style="color:rgb(255, 255, 255, 0.5);">Keine Podcasts gefunden.</p>';
        else podcasts.forEach((podcast, index)=>{
            const podcastItem = createPodcastItem(podcast);
            if (index % 2 !== 0) podcastItem.classList.add('result-podcast-new-background');
        ...
        resultsContainer.innerHTML = '<p style="color:rgb(255, 255, 255, 0.5);">Fehler beim Laden der Podcasts. Bitte versuchen Sie es sp\xe4ter erneut.</p>';
    }
}
...
//# sourceMappingURL=public.f1954841.js.map
