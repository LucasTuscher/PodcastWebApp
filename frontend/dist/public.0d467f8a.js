async function loadSlider() {
    ...
    try {
        ...
        if (!response.ok) throw new Error('Fehler beim Abrufen der Podcast-Daten: ' + response.statusText);
...
function calculateProgressBar(progressBar) {
    ...
    for(let i = 0; i < progressBarItemCount; i++){
        ...
        if (i === sliderIndex) barItem.classList.add("active");
...
function onHandleClick(handle) {
    ...
    const itemCount = slider.children.length;
    const progressBarItemCount = Math.ceil(itemCount / itemsPerScreen);
    if (handle.classList.contains("left-handle")) sliderIndex = (sliderIndex - 1 + progressBarItemCount) % progressBarItemCount;
    if (handle.classList.contains("right-handle")) sliderIndex = (sliderIndex + 1) % progressBarItemCount;
    ...
    progressBar.querySelectorAll(".progress-item").forEach((item, index)=>{
        if (index === sliderIndex) item.classList.add("active");
        else item.classList.remove("active");
    });
}

//# sourceMappingURL=public.0d467f8a.js.map
