...
function searchPodcasts() {
    ...
    statusMessage.textContent = "Suche l\xe4uft...";
    results.innerHTML = '';
    closeButton.style.display = 'block';
    resultCount = 0;
    fetch(`https://api.fyyd.de/0.2/search/podcast?title=${encodeURIComponent(query)}&count=${count}`).then((response)=>{
        ...
        if (resultCount === 0) statusMessage.textContent = 'Keine Ergebnisse gefunden.';
        else statusMessage.textContent = `${resultCount} Ergebnisse gefunden.`;
    }).catch((error)=>{
...
function checkScreenWidth() {
    if (window.innerWidth < 650) resetSearch();
}
...
//# sourceMappingURL=public.8c99972d.js.map
