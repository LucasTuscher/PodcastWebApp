async function checkLoginStatusLogoutButton() {
    try {
        ...
        if (loginResponse.ok) {
            ...
            if (loginData.loggedIn) logoutButton.style.display = 'inline-block'; // Zeige den Button
            else logoutButton.style.display = 'none'; // Verstecke den Button
        } else {
...
async function navigateToUserOrLogin() {
    try {
        ...
        if (loginResponse.ok) {
            const loginData = await loginResponse.json();
            if (loginData.loggedIn) // Wenn der Benutzer eingeloggt ist, leite ihn zur User Admin-Seite weiter
            navigateTo('account-administration');
            else // Wenn der Benutzer nicht eingeloggt ist, leite ihn zur Login-Seite weiter
            navigateTo('login');
        } else // Fehler beim Abrufen des Login-Status, leite den Benutzer zur Login-Seite weiter
        // console.log("Fehler beim Abrufen des Login-Status")
        navigateTo('login');
    } catch (error) {
        console.error("Fehler beim \xdcberpr\xfcfen des Login-Status:", error);
        // Bei Fehler gehe zum Login
        navigateTo('login');
    }
}

//# sourceMappingURL=public.e6b0d7ee.js.map
