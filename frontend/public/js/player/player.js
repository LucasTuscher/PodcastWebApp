...
function renderEpisodes(podcastData) {
    ...
    if (playFirstEpisodeButton) {
        playFirstEpisodeButton.addEventListener('click', function() {
...
function playEpisode(index, podcastData, episodeMP3) {
    ...
    const sliderEl = document.querySelector("#fullscreen-player-range-js");
    if (sliderEl) {
        currentPlayer.volume = sliderEl.value / 50;
    }
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (fullscreenPlayButton) {
        fullscreenPlayButton.addEventListener('click', togglePlay);
    }
});
...
function playNextEpisode() {
    if (currentPlayer && currentPodcastData) {
        ...
        if (currentEpisodeIndex < currentPodcastData.episodes.episodes.length - 1) {
            playEpisode(currentEpisodeIndex + 1, currentPodcastData, currentPodcastData.episodes.episodes[currentEpisodeIndex + 1].enclosure);
        }
    }
}
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (forwardButton) {
        forwardButton.addEventListener('click', playNextEpisode);
    }

    if (backwardButton) {
        backwardButton.addEventListener('click', playPreviousEpisode);
    }

    if (fullscreenForwardButton) {
        fullscreenForwardButton.addEventListener('click', playNextEpisode);
    }

    if (fullscreenBackwardButton) {
        fullscreenBackwardButton.addEventListener('click', playPreviousEpisode);
    }
});
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (backPodcastButton) {
        backPodcastButton.addEventListener('click', async function() {
            if (currentPodcastData) {  // Überprüfen, ob ein Podcast geladen ist
                try {
                ...
                console.log('Kein Podcast ist geladen.');
            }
        });
    }
});
